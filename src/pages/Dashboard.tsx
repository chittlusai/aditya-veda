    ...
    <div className="space-y-6 pb-12 max-w-7xl mx-auto text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-slate-950 border border-slate-800 text-white shadow-xl">
        ...
        <div className="w-full md:w-96">
          <form onSubmit={handleQuickSubmit} className="flex items-center bg-white rounded-xl p-1.5 border border-slate-300 shadow-sm">
            <Search size={16} className="text-slate-400 ml-2.5 shrink-0" />
            <input
              type="text"
              value={quickInput}
              onChange={(e) => {
              ...
              placeholder="Scan URL, domain, or message in real time..."
              className="w-full bg-transparent px-2.5 py-1.5 text-xs text-slate-900 placeholder-slate-400 outline-none font-mono"
            />
            <button
              type="submit"
              className="btn-primary text-xs px-3.5 py-2 rounded-lg shrink-0 font-bold cursor-pointer font-mono shadow-xs"
            >
              Scan Now
            </button>
          </form>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statsCards.map((s) => (
          <div
            key={s.label}
            className={`card bg-white border border-slate-200/90 shadow-sm p-4 rounded-2xl transition-all ${s.glow}`}
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{s.label}</span>
              <div className={`p-1.5 rounded-lg bg-slate-100 ${s.color}`}>
                <s.icon size={16} />
              </div>
            </div>
            ...
            <div className="text-[11px] font-medium text-slate-500 mt-1 flex items-center gap-1 font-mono">
      ...
      <div>
        <div className="flex items-center justify-between mb-3.5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900">Active Defense Scanners</h2>
          </div>
          ...
          {TOOLS.map((t) => (
            ...
            >
              <SpotlightCard
                ...
                className="p-5 h-full rounded-2xl bg-white border border-slate-200/90 shadow-xs hover:border-orange-400/60 hover:shadow-md transition-all text-left flex flex-col justify-between"
              >
                <div>
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              to="/scans"
              className="text-xs font-mono font-bold text-orange-600 hover:text-orange-700 flex items-center gap-1 shrink-0 ml-2"
            >
              Full History →
            </Link>
          </div>
